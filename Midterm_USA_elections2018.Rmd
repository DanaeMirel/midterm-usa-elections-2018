...
corr_pal <- viridis_pal(direction = 1)(20)
...
senate_winners %>%
  ...
  add_trace(z = ~as.numeric(party), locations = ~state,
    colors = c("blue", corr_pal[16], "red"),
    #colors = c(corr_pal[2], corr_pal[12], corr_pal[20]),
    hoverinfo = "text",
    text = ~paste("Candidate:", name, "<br>",
                  "Party:", party, "<br>",
...
senate_vote %>%
  ...
  layout(title = " Votes that the Republican candidate received according to the pre-recount in Florida", xaxis = map_axes, yaxis = map_axes)
